...
@registry.register("last_touch")
class LastTouchAttribution(BaseAttributionModel):
    name = "last_touch"
    description = "末次触碰归因——100% 功劳归最后一个触点"
    ...
    async def calculate(
        ...
        **params,
        ...
        """
        对每条转化记录，取该用户时间上最晚的触点，将转化价值全部分配给它。
        ...
        Returns:
            {"model": "last_touch", "attributions": {channel: value}, "summary": ...}
        """
        ...
        # 每个 conversion_id 取最后一个（最晚）触点
        last_idx = df.groupby("conversion_id")["timestamp"].idxmax()
        last_touchpoints = df.loc[last_idx]

        # 按渠道汇总
        summary_value = last_touchpoints.groupby("channel")["value"].sum().to_dict()
        ...
        return {
            ...
            "summary": {
                ...
                "touchpoint_count": int(len(last_touchpoints)),
                "channel_breakdown": {
